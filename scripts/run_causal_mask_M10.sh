#!/usr/bin/env bash
# End-to-end M10 evaluation for a causal-mask-trained GDN_GDeltaUQ.
#
# Usage:
#   SEED=42 GPU=0 bash scripts/run_causal_mask_M10.sh
#
# Stages (each resumable if its output exists):
#   1. Train base GDN_GDeltaUQ with -causal_mask
...
RUN_TAG="${RUN_TAG:-causalmask_sw${SW}_seed${SEED}}"
PRE_DIR="pretrained/swat_gdeltauq_${RUN_TAG}"
RES_DIR="results/causal_mask_M10/${RUN_TAG}"
...
echo "================================================================="
echo "[$(date -Iseconds)] START seed=$SEED gpu=$GPU sw=$SW ep=$EP K=$K"
echo "  RUN_TAG=$RUN_TAG"
echo "================================================================="

CMASK="data/swat/causal_adjacency.npy"
...
if [[ -n "$CKPT" && -n "$HP" ]]; then
  ...
  echo "[stage 1] train base GDN_GDeltaUQ with causal mask"
  CUDA_VISIBLE_DEVICES="$GPU" "$PY" train_gdeltauq_main.py \
    ...
    -causal_mask "$CMASK" -causal_mask_keep_self 1 \
    -device cuda:0
...
# ---------------------------------------------------------------------------
# Fusion uses a different split schema than the gdeltauq_split.json the
# training script consumes -- it operates on TEST-csv row indices and
# expects C_row_range + labeled_val_range. The ensemble pipeline persists
# this file alongside its calibration bundle; reuse it.
...
echo "================================================================="
echo "[$(date -Iseconds)] DONE seed=$SEED  results in $RES_DIR"
echo "================================================================="
