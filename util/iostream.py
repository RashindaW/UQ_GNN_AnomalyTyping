...
def save_attack_infos(f1_scores, total_err_scores, labels, names, save_path, dataset, config):
    ...
    for head, end in attack_inters:
        ...
        sorted_attack_infos = {
            k: v for k, v in sorted(attack_infos.items(), reverse=True, key=lambda item: item[1])
        }
        sorted_topk_attack_infos = {
            k: v for k, v in sorted(
                topk_attack_infos.items(), reverse=True, key=lambda item: item[1]
            )
        }
