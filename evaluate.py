from util.data import eval_scores, get_err_median_and_iqr, eval_mseloss
import numpy as np
...
def get_err_scores(test_res, val_res):
    ...
    # Fit median+IQR on VALIDATION residuals (paper Section 3.6 eq. 12), not on
    # test residuals — using test residuals leaks the test distribution into
    # the standardisation. The upstream d-ailin/GDN repo ships this bug; this
    # is the one-line fix.
...
def get_f1_scores(total_err_scores, gt_labels, topk=1):
    ...
    for i, indexs in enumerate(topk_indices):
        sum_score = sum(
            score for k, score in enumerate(
